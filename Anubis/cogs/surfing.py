    ...
    async def define(self, ctx, *, word=f""):
        if word.strip().replace(" ", "") != "":
            try:
                ...
                embed = discord.Embed(
                    title=f"{word.title()}", description=f'{definition}', color=discord.Colour.blue())
                await ctx.send(embed=embed)
            except AttributeError:
                embed = discord.Embed(
                    title="Issue", description=f'"{word}" was not found.', color=discord.Colour.orange())
                await ctx.send(embed=embed)
        else:
            embed = discord.Embed(
                title="Issue", description="You must specify a word.", color=discord.Colour.orange())
            await ctx.send(embed=embed)
